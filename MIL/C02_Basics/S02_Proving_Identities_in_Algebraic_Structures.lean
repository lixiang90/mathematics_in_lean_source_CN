...
从数学上讲，一个环由一组对象 :math:`R`、操作 :math:`+`、:math:`\times` 和常数 :math:`0` 和 :math:`1` 组成，以及一个操作 :math:`x \mapsto -x`，满足以下条件：

* :math:`R` 与 :math:`+` 构成一个 *阿贝尔群*，其中 :math:`0` 是加法单位元，负数是其逆元。
* 乘法是可结合的，且具有单位元 :math:`1`，并且乘法分配于加法之上。
...
/- TEXT:
关于第一行中的方括号，您稍后会了解更多，但暂且可以说，这个声明给了我们一个类型 ``R``，以及 ``R`` 上的一个环结构。
然后，Lean 允许我们使用 ``R`` 元素的通用环符号，并利用一个关于环的定理库。

一些定理的名称可能会让您感到熟悉：它们正是我们在上一节中用来计算实数的定理。
Lean 不仅适用于证明关于具体数学结构（如自然数和整数）的事实，还适用于证明关于抽象结构的事实，这些抽象结构可以通过公理来表征，比如环。
此外，Lean 支持对抽象和具体结构进行 *通用推理*，并且可以被训练来识别适当的实例。
因此，关于环的任何定理都可以应用于具体的环，如整数 ``ℤ``、有理数 ``ℚ`` 和复数 ``ℂ``，也可以应用于扩展环的抽象结构的任何实例，例如任何有序环或任何域。

.. index:: commutative ring

然而，并非所有实数的重要性质在任意环中都成立。
例如，实数上的乘法是交换的，但一般情况下并不成立。
如果您曾学过线性代数课程，您会意识到，对于每个 :math:`n`，由实数组成的 :math:`n` 乘以 :math:`n` 的矩阵形成一个环，在这个环中通常不满足交换性质。
事实上，如果我们声明 ``R`` 是一个 *可交换* 环，则上一节中的所有定理在用 ``R`` 替换 ``ℝ`` 时仍然成立。
TEXT. -/
section
-- QUOTE:
...
/- TEXT:
我们把其余的证明留给您来检查是否没有改变。
注意，当一个证明很简短时，比如 ``by ring`` 或 ``by linarith`` 或 ``by sorry``，将其放在与 ``by`` 同一行上是常见的（也是允许的）。
良好的证明写作风格应该在简洁性和可读性之间取得平衡。

本节的目标是加强您在上一节中所发展的技能，并将它们应用于对环进行公理推理。
我们将从上面列出的公理开始，利用它们推导出其他事实。
我们证明的大部分事实已经在 Mathlib 中了。
我们将给我们证明的版本相同的名称，以帮助您学习库的内容以及命名惯例。

.. index:: namespace, open, command ; open

Lean 提供了一种类似于编程语言中使用的组织机制：当一个定义或定理 ``foo`` 被引入到 *命名空间* ``bar`` 中时，它的完整名称是 ``bar.foo``. 
稍后的命令 ``open bar`` 会 *打开* 这个命名空间，这样我们就可以使用更短的名称 ``foo``. 
为了避免由于名称冲突而产生的错误，在下一个示例中，我们将我们对库定理的版本放在一个名为 ``MyRing.`` 的新命名空间中。

下一个示例显示，我们不需要 ``add_zero`` 或 ``add_right_neg`` 作为环公理，因为它们可以由其他公理推导出来。
TEXT. -/
-- QUOTE:
namespace MyRing
variable {R : Type*} [Ring R]
...
（如果您注意到了，我们已经将 ``(R : Type*)`` 中的圆括号改为了 ``{R : Type*}`` 中的花括号。这声明了 ``R`` 是一个 *隐式参数*。我们将在稍后解释这意味着什么，但在此期间不用担心它。）
...
/- TEXT:
经过足够的规划，您可以使用三次重写来完成每一个。

.. index:: implicit argument

现在我们来解释一下花括号的用法。
假设您处于这样一种情况：在您的上下文中有 ``a``、 ``b`` 和 ``c``，以及一个假设 ``h : a + b = a + c``，
您想要得出结论 ``b = c``. 在 Lean 中，您可以将定理应用于假设和事实，就像您可以将它们应用于对象一样，因此您可能认为 ``add_left_cancel a b c h`` 是事实 ``b = c`` 的一个证明。
但请注意，明确地写出 ``a``、 ``b`` 和 ``c`` 是多余的，因为假设 ``h`` 明确指出了我们所关注的对象。
在这种情况下，输入一些额外的字符并不困难，但如果我们想将 ``add_left_cancel`` 应用于更复杂的表达式，写起来将会很繁琐。
在这种情况下，Lean 允许我们将参数标记为 *隐式的*，意味着它们应该被省略，并由其他方法推断出来，比如后续的参数和假设。
...
虽然 ``exact`` 策略在技术上是多余的，因为它比 ``apply`` 更弱，但它使证明脚本对人类读者略微更清晰，并且在库演变时更易于维护。
...
/- TEXT:
到目前为止，您应该也能够在下一个练习中用一个证明替换每个 ``sorry``，仍然只使用我们在本节中建立的关于环的事实。
TEXT. -/
-- QUOTE:
...
证明项 ``rfl`` 是“自反性”的简写。将其呈现为 ``a - b = a + -b`` 的证明，强迫 Lean 展开定义并将两边识别为相同。 
``rfl`` 策略也是如此。这是 Lean 底层逻辑中已知的一种*定义等式*的实例。这意味着不仅可以使用 ``sub_eq_add_neg`` 进行重写，
以替换 ``a - b = a + -b``，而且在某些上下文中，处理实数时，您可以互换等式的两边。例如，您现在有足够的信息来证明上一节中的定理 ``self_sub``：
TEXT. -/
-- BOTH:
namespace MyRing
variable {R : Type*} [Ring R]
...
/- TEXT:
展示您可以使用 ``rw`` 来证明这一点，但如果您将任意环 ``R`` 替换为实数，则还可以使用 ``apply`` 或 ``exact`` 来证明它。

Lean 知道在任何环中都成立 ``1 + 1 = 2``。通过一点努力，您可以利用这一点来证明上一节中的定理 ``two_mul``：
TEXT. -/
-- QUOTE:
-- BOTH:
...
我们结束本节时需要注意的是，我们上面建立的关于加法和取反的一些事实并不需要完整的环公理，甚至不需要加法的可交换性。弱一些的 *群* 的概念可以被公理化如下：
TEXT. -/
section
-- QUOTE:
...
/- TEXT:
如果你感到自信，尝试使用这些公理来证明关于群的以下事实。您将需要在途中证明一些辅助引理。我们在本节中进行的证明提供了一些提示。
TEXT. -/
-- BOTH:
namespace MyGroup
...
这在一定程度上是出于历史原因，但也为了方便使用更短的名称来表示处理交换环的策略，因为它使用得更频繁。
TEXT. -/
