...
我们也不预设任何对形式化方法的背景知识。形式化可以被视为一种计算机编程：我们将用一种规范化的语言，类似于编程语言，
在 Lean 可以理解的语言中编写数学定义、定理和证明。
作为回报，Lean 提供反馈和信息，解释表达式并保证它们是良好形式的，并最终证明我们的证明的正确性。

你可以从 `Lean 项目页面 <https://leanprover.github.io>`_
和 `Lean 社区网页 <https://leanprover-community.github.io/>`_
了解更多关于 Lean 的信息。本教程基于 Lean 的庞大且不断增长的库 *Mathlib*。
我们也强烈建议加入 `Lean Zulip 在线聊天群 <https://leanprover.zulipchat.com/>`_，如果你还没有加入的话。
在那里你会发现一个充满活力和友好的 Lean 爱好者社区，愿意回答问题并提供精神支持。

虽然你可以在线阅读此书的 pdf 或 html 版本，但它设计为可以与 Lean 在 VS Code 编辑器中交互式阅读。开始学习吧：
...
我们强烈建议复制该文件夹，并在复制的文件夹中进行实验和练习。这样可以保持原始文件的完整性，并且在更新存储库时更容易操作（见下文）。
你可以将复制的文件夹命名为 ``my_files`` 或其他任何你喜欢的名字，并在其中创建自己的 Lean 文件。

在那时，你可以按照以下步骤在 VS Code 的侧边栏中打开教科书：

1. 输入 ``ctrl-shift-P`` （在 macOS 中为 ``command-shift-P`` ）。

2. 在出现的栏中输入 ``Lean 4: Open Documentation View``，然后按回车键。（一旦在菜单中被突出显示，你就可以按回车键选择它。）

3. 在打开的窗口中，点击 ``Open documentation of current project``。

或者，你可以在云中运行 Lean 和 VS Code，使用 `Gitpod <https://gitpod.io/>`_。
...
然后跟着输入 ``lake exe cache get`` 来更新存储库。（这假设你没有改变 ``MIL`` 文件夹的内容，这也是我们建议你制作副本的原因。）
...
此外，本书还提供了许多具有挑战性的练习供你尝试。不要匆忙跳过这些练习！Lean 是关于*交互式地做*数学，而不仅仅是阅读。
通过练习来工作是体验的核心。你不必完成所有练习；当你觉得已经掌握了相关技能时，可以自由地继续前进。
